/**
 * Runs controller
 * - POST /:experimentId/run  -> starts a run
 * - GET  /:runId             -> returns run with responses + metrics
 * - GET  /:runId/stream      -> SSE stream for progress/completion
 */
...
runsRouter.get("/:runId/stream", async (req, res) => {
  ...
  req.on("close", () => {
    // Allow GC to collect listeners when client disconnects.
  });
});
